"""
Multi-model adversarial ranking.
...
Naming note: this module previously exposed ``Leaderboard*`` symbols.
Phase 10 (T3) introduced a separate ``toki.leaderboard`` module backed
by SQLite for persistent score tracking; what was always a "ranking"
operation now lives under that name to free the leaderboard namespace.
...
@dataclass
class RankingEntry:
    """A single model's position in the ranking."""
...
class RankingConfig:
    ...
    output_dir: str = "experiments/rankings"


@dataclass
class RankingResult:
    """Full ranking result — rankings, all pairwise outcomes, config snapshot."""

    name: str
    timestamp: str
    config: dict
    entries: List[RankingEntry]              # ordered by rank (ascending)
    ...
    def save(self, base_dir: Optional[str] = None) -> Path:
        """Persist to ``<base_dir>/<timestamp>_<name>/ranking.json``.
        ...
        """
        out_dir = Path(base_dir or self.config.get("output_dir", "experiments/rankings"))
        run_dir = out_dir / f"{self.timestamp}_{self.name}"
        run_dir.mkdir(parents=True, exist_ok=True)
        out = run_dir / "ranking.json"
        if out.exists():
            raise FileExistsError(f"Ranking result already exists at {out}")
        out.write_text(json.dumps(asdict(self), indent=2))
        return out

    @classmethod
    def load(cls, path) -> "RankingResult":
        """Load from a previously saved ``ranking.json``."""
        data = json.loads(Path(path).read_text())
        data["entries"] = [RankingEntry(**e) for e in data["entries"]]
    ...
    def format_table(self) -> str:
        """Return a human-readable ranking table (ASCII, no external deps)."""
        lines: List[str] = []
        sep = "-" * 72
        lines.append(sep)
        lines.append(
            f"  Ranking: {self.name}   ({self.timestamp})"
        )
        lines.append(
...
def _rank_entries(
    ...
    """Build :class:`RankingEntry` objects and assign 1-indexed ranks.
...
class Ranking:
    ...
    Usage::
        ...
        from toki.ranking import Ranking, RankingConfig
        from toki.compare import BASELINES, ModelSpec

        models = [ModelSpec(name, fn) for name, fn in BASELINES.items()]
        cfg = RankingConfig(name="demo", seed=42)
        r = Ranking(models, cfg)
        result = r.run(save=True)
    ...
    def __init__(
        ...
        if len(models) < 2:
            raise ValueError("Ranking requires at least 2 models")
    ...
    def run(self, save: bool = False) -> RankingResult:
        """Evaluate all models on the same dataset and build the ranking.

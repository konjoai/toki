"""Tests for toki.ranking — multi-model adversarial ranking."""
...
def _small_cfg(**kw) -> RankingConfig:
    """Return a tiny RankingConfig for fast tests."""
    defaults = dict(
        name="test_lb",
        seed=7,
        jailbreak_count=3,
...
# ---------------------------------------------------------------------------
# Test 4: RankingEntry dataclass construction
# ---------------------------------------------------------------------------
...
def test_three_model_leaderboard_structure():
    ...
    cfg = _small_cfg()
    lb = Ranking([SAFE, UNSAFE, MIXED], cfg)
    result = lb.run()
...
def test_save_and_load_round_trip(tmp_path):
    ...
    out = tmp_path / f"{result.timestamp}_persistence_test" / "ranking.json"
    assert out.exists()

    loaded = RankingResult.load(out)
    ...
    assert isinstance(loaded.entries[0], RankingEntry)
...
def test_format_table_contains_all_models():
    cfg = _small_cfg()
    result = Ranking([SAFE, UNSAFE, MIXED], cfg).run()
...
def test_leaderboard_result_config_snapshot():
    """The config dict inside RankingResult must round-trip to RankingConfig."""
    cfg = _small_cfg(alpha=0.01, name="cfg_snapshot")
    result = Ranking([SAFE, UNSAFE], cfg).run()
    snap = result.config

...
ALL_PRESENT=true
check_tool() { command -v "$1" &>/dev/null && ok "$1" || { err "$1 not found — $2"; ALL_PRESENT=false; }; }
...
$ALL_PRESENT && echo -e "${GRN}${BOLD}Framework installed.${RST}" || echo -e "${YEL}${BOLD}Some tools missing.${RST}"
echo "Run: git commit --allow-empty -m 'test: verify konjo hooks'"
